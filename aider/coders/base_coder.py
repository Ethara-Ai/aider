...
class Coder:
    ...
    def calculate_and_show_tokens_and_cost(self, messages, completion=None):
        ...
        self.message_tokens_reasoning += reasoning_tokens
        ...
        if self.message_tokens_reasoning:
            tokens_report += (
                f" ({format_tokens(self.message_tokens_reasoning)} reasoning)"
            )

        elapsed = (
            time.time() - self.message_start_time if self.message_start_time else 0.0
        )
        self.message_start_time = 0.0
        self.message_elapsed_time += elapsed
        self.total_time += elapsed
        if elapsed >= 60:
            minutes = int(elapsed // 60)
            seconds = elapsed % 60
            time_report = f"Time: {minutes}m {seconds:.1f}s"
        else:
